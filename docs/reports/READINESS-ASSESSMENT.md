...
| R16 | O `ndt_version_ref` assinado identifica o NDT por **nome e versão**, não pelos seus bytes — o hash do NDT só existe no manifesto do `.ndfpkg`, fora dos bytes cobertos pela assinatura do NDF-core (SPEC.md §1.1). Reproduzido: substituir texto fixo do NDT mantendo nome/versão e recalculando apenas o hash do manifesto é aceite pelo validador, com os mesmos bytes de NDF-core e envelope. Corrige a leitura de §4.2.1, que tratava "versão exata do template" como equivalente a bytes vinculados | Alto — bloqueador de confiança na reconstituição documental; é a junta que falta para que "o que se assina" e "o que se vê" sejam a mesma coisa | **resolvido** (2026-09-11) — [ADR-026](../architecture/ADR-026-dependencias-interpretacao-autenticadas.md); vetor `PKG-NEG-017` reproduz o ataque original e é rejeitado. Ver R23/R24 para dependências que ficaram, à parte desta, sem a mesma proteção na primeira ronda |
| R17 | `rfc8785` é dependência opcional em `tools/validate.py` (import em `try/except`, `rfc8785 = None` em falha); se ausente, a verificação de bytes canónicos JCS é omitida sem qualquer sinal no resultado. Reproduzido simulando a ausência da biblioteca | Alto — uma verificação obrigatória pode desaparecer silenciosamente sem que o `PASS` o assinale | **resolvido** (2026-09-12) — `rfc8785` passa a dependência obrigatória (falha dura no arranque, mesmo padrão de todas as outras ferramentas do projeto); vetor `tools/check_jcs_required.py` |
| R18 | O `PASS` global do validador não distingue estrutura, canonicalização, integridade de componentes, autenticação de dependências, assinatura/confiança e representação; o pacote de exemplo (`envelope.json`) recebe `PASS` com assinaturas, certificados e timestamps *placeholder*, sem sinalização de que não são reais | Médio-alto — um `PASS` sem camadas é fácil de ler como mais do que verifica | **resolvido** (2026-09-12) — `validate_package_report()` devolve resultado por camada (estrutura, canonicalização, integridade de componentes, dependências, assinatura/confiança, representação); `assinatura_confianca` nunca reporta "aprovada" (sem verificação criptográfica real) e sinaliza explicitamente marcadores de placeholder; `--json` expõe versão do verificador, perfil, instante e política de confiança; SPEC.md §9.4.2; vetor `tools/check_layered_report.py` |
...
**Terceira ronda — revisão adversarial ao commit `3985001` (2026-09-11).**
A primeira implementação de `dependencias_interpretacao` (que fechou `R16`)
foi, ela própria, sujeita a testes adversariais reproduzidos localmente,
com resultado: o ataque original ao NDT está de facto fechado, mas duas
outras dependências continuavam vulneráveis à mesma classe de ataque
(substituir bytes, recalcular só `manifest.json`), e uma promessa da
especificação não correspondia ao comportamento do código:

- trocar `recursos/brasao-republica.svg` (recurso do NDT) dava `PASS` — `R23`;
- trocar `schemas/oficio.schema.json` (schema de tipo **canónico**, fora da
  condição original de `schema_tipo`) dava `PASS` — novo achado, `R24`;
- renomear o NDT mantendo bytes corretos era **rejeitado**, apesar de
  ADR-026 e SPEC.md §2.6.2 afirmarem resolução por `ref`, não por caminho —
  divergência de contrato, não falha criptográfica.

Os três foram verificados de forma independente antes de qualquer correção
(não só aceites da descrição do utilizador), e todos os três estão
corrigidos e cobertos por vetor de teste nesta mesma ronda — ver R23, R24 e
[ADR-026 §"Correções"](../architecture/ADR-026-dependencias-interpretacao-autenticadas.md#correções-segunda-ronda-2026-09-11).
...
**R17/R18 — resolução (2026-09-12).** `rfc8785` passa a falha dura no
arranque de `tools/validate.py`, alinhando-o com todas as outras ferramentas
do projeto (nenhuma outra tratava esta dependência como opcional).
`validate_package_dir` passa a `validate_package_report`, que devolve
resultado por camada — `estrutura`, `canonicalizacao`,
`integridade_componentes`, `dependencias_interpretacao`,
`assinatura_confianca`, `representacao` — com um princípio central: a
camada `assinatura_confianca` **nunca** chega a "aprovada", porque este
projeto não tem biblioteca CAdES nem *trust store*; no máximo,
"indeterminada", com sinalização explícita quando o material contém
marcadores de placeholder. `representacao` fica sempre "não_executada" —
este verificador não renderiza. `--package <dir> --json` expõe o relatório
completo, com versão do verificador, perfil, instante de verificação e uma
declaração explícita da política de confiança (nenhuma). SPEC.md §9.4.2
documenta isto como recomendação para qualquer verificador, não só para
`tools/validate.py`. Vetores: `tools/check_jcs_required.py` (R17),
