...
| Pacote | exemplo positivo e vetores negativos de inventário, caminhos, NDT e envelope | 1 positivo + 22 negativos concluídos (`tools/check_package_vectors.py`) |
| Perfis de avaliação | coerência entre `pattern` e `examples[]` de cada schema de perfil | 8 exemplos + 9 casos negativos (`tools/check_profile_patterns.py`) |
